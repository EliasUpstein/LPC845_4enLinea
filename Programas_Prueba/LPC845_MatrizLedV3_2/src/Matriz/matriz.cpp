 ...
 *
 * Siempre que el tiempo sea corto, el tiempo se consume por la propia ejecuci{on del micro
 * */
void show(void)
{
	...
	//si el bit es 0 => TOH = 400ns
	//si el bit es 1 => T1H = 800ns

	//Si el primer bit a enviar es un 1
	if((matriz[0] >> 23) & 0x1)
		DELAY_ASM
	...
	while(block)
	{
		...
		switch (estado)
		{
			case NIVEL_ALTO:
				...
				//si el bit es 0 => TOL = 850ns
				//si el bit es 1 => T1L = 450ns
