...
/**
	...
	\Esto está mal, el FRO es por default 24MHz y el divisor de la CPU en 2 genera 12MHZ de main clock
	\ Pero si en la primera instrucción seteamos el divisor en x 1, entonces la frecuencia de CPU es la de FRO de 24Mhz
	\ Si multiplicamos  x4 y dividimos x 2 da 48MHz de CPU.
...
void Inicializar_PLL( void )
{
	...
	SYSCON->MAINCLKSEL               &= ~(0x03);  //set "main clock" to IRC (FRO) oscillator, if not system will lock up when PLL turns off!(sec. 3.5.11)
	...
	SYSCON->SYSPLLCLKSEL             = 0;      	  //connect IRC (FRO) oscillator to SYSTEM PLL (sec. 3.5.9)
